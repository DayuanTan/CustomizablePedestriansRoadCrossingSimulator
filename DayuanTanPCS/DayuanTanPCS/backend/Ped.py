...
class Ped:
    ...
    def set_initial_standing_position(self, params):
        ...
        while is_conflict:
            x_offset = 0
            if params.distribution_type == 1: # normal distribution
            ...
            elif params.distribution_type == 2: # poisson distribution
                # poisson cannot find initial positions for large number of peds. So add a random to it.
                random_index = random.randrange(0, 999)
                x_offset = poisson.rvs(params.waiting_area_length / 2, size=999, loc=random_index,  random_state=None)[random_index] + random.randrange(0-params.waiting_area_length/2, params.waiting_area_length/2)
                newy = poisson.rvs(params.waiting_area_position_y_mean, size=999, loc=random_index,  random_state=None)[random_index] + random.randrange(0-params.waiting_area_position_y_mean, params.waiting_area_position_y_mean)
            elif params.distribution_type == 3: # t distribution
                df = 10 / params.total_size # degree of freedom (df) 
                x_offset = abs(t.rvs(df, size=1)[0]) 
                newy = t.rvs(df, size=1)[0] + params.waiting_area_position_y_mean
            # print(is_conflict, x_offset, newy)
            ...
            if self.direction == "left2right":
                if not self.is_newposition_conflicted(newx, newy, 0, "standing", params, attempt_ctr // 10):
                    is_conflict = False
            elif self.direction == "right2left":
                if not self.is_newposition_conflicted(newx, newy, 0, "standing", params, attempt_ctr // 10):
                    is_conflict = False
    ...
    def is_y_inside_bound(self, newy, params) -> bool:
        if newy > params.crosswalk_width + params.outside_margin_width or newy < - params.outside_margin_width:
            return False
        return True
    def is_x_inside_bound(self, newx, params) -> bool:
        if newx < 0 - params.outside_margin_width or newx > params.total_length + params.outside_margin_width:
            return False
        return True
    ...
    def is_newposition_conflicted(self, newx: int, newy: int, real_radius, mode: str, params, attempt_ctr_compromise) -> bool:
        if not self.is_y_inside_bound(newy, params):
            return True
        if not self.is_x_inside_bound(newx, params):
            return True
        ...
        for another in used_all_peds:
            ...
            real_distance = math.sqrt((newx - another.x)**2 + (newy - another.y)**2)
            distance = real_distance * (attempt_ctr_compromise/10)
    ...
    def generate_rightback_newpositions(self, params):
        offset = self.speed * params.step_time / 100
        all_newpositions_rightback = list()

        for i in range(100):
            circle_radius = self.speed * params.step_time - offset * i
            farthest_newy = (self.y - circle_radius ) if self.direction == "left2right" else (self.y + circle_radius )
            farthest_newx = self.x
            all_newpositions_rightback.append([farthest_newx, farthest_newy, i])

        for i in range(1, 99):
            newx = farthest_newx - offset * i if self.direction == "left2right" else farthest_newx + offset * i
            circle_radius_sq = circle_radius ** 2
            x_x0_sq = (newx - self.x)**2
            if circle_radius_sq - x_x0_sq < 0:
                continue
            sqrt_abs = math.sqrt(circle_radius_sq - x_x0_sq)
            newy =  self.y - sqrt_abs if self.direction == "left2right" else (self.y + sqrt_abs )

            for j in range(1, 99):
                all_newpositions_rightback.append([newx * (100 - j)/100, newy * (100 - j)/100, j])
        return all_newpositions_rightback
    ...
    def move_one_step(self, params):
        ...
        while counter >= 0 and counter < 100:
            ...
            for newx, newy, new_pos_id_when_move_into_bound in all_newpositions:
                ...
                if not self.is_newposition_conflicted(newx, newy, real_occupied_area_radius, "moving", params, 10): # 10 is not compromise
        ...
        if self.nomove_counter >= 4: # wait too long, no move too long, so move right
            all_newpositions = self.generate_rightback_newpositions(params)
            for newx, newy, i in all_newpositions:
                real_occupied_area_radius = self.radius_moving - ( (self.radius_moving - self.radius_standing)/100 ) * i
                if not self.is_newposition_conflicted(newx, newy, real_occupied_area_radius, "moving", params, 10): # 10 is not compromise
                    self.x = newx
                    self.y = newy
                    self.status = "moving" # move as designed
