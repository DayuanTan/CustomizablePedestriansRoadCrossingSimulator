...
def run():
    ...
    while traci.simulation.getMinExpectedNumber() > 0 : # run until all v and ped have arrived. 
        ...
        if phaseNoCurr == 0 and (phaseNoPrevious == 2 or phaseNoPrevious == 3): 
            ...
            greenDur1 = vNumOther1 / 4 * 4 + vNumTurnLeft2 * 4 # 4lanes, avg 4 seconds per veh to pass intersection
            greenDur2 = vNumOther2 / 4 * 4 + vNumTurnLeft1 * 4
...
if __name__ == "__main__":
    ...
    tripinfo_output_filename = "output/2ndexperiment.tripinfo." +dt_string+".xml"
    emission_output_filename = "output/2ndexperiment.emission." +dt_string+".xml"
    ped_waitingtime_output_filename = "output/2ndexperiment.ped_waitingtime." +dt_string+".xml"
